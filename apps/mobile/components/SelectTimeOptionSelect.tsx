...
interface SelectTimeOptionSelectProps {
  selectTimeOption: "my" | "entire";
  handleSelectOption: (option: "my" | "entire") => void;
}

function SelectTimeOptionSelect({ selectTimeOption, handleSelectOption }: SelectTimeOptionSelectProps) {
  const unSelectedTimeOption = selectTimeOption === "my" ? "entire" : "my";
  ...
  return (
    <div>
      ...
      <div
        className={`transition-opacity duration-200 ${isSelectOpen ? "opacity-100" : "opacity-0"} ${isSelectOpen ? "max-h-[100px]" : "max-h-0"} overflow-hidden`}
      >
        {isSelectOpen && (
          <button
          ...
          >
            {selectTimeOption === "my" ? "전체 가능 시간" : "나의 가능 시간"}
          </button>
        )}
      </div>
    </div>
